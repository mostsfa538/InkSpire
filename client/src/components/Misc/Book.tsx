	...
	return (
		<div className={`flex ${direction} items-center gap-2`}>
			<div className={`${style} bg-white z-20 relative`}>
				...
				<div
					onMouseLeave={() => dispatch(toggleAddToCart(false))}
					className="absolute flex flex-col text-white w-full h-full top-0 opacity-0 p-2 transition-all hover:opacity-100 max-lg:opacity-100">
					{user && direction === "flex-col" && (
						<div className="justify-between text-sm flex transition-all [&>*]:rounded-full [&>*]:bg-black [&>*]:bg-opacity-50 [&>*]:p-2 [&>*]:h-full [&>*]:flex [&>*]:justify-center z-10">
							...
							<button
								onClick={() => dispatch(toggleAddToCart(!displayAddToCart))}>
								<BiShoppingBag />
							</button>
						</div>
					)}
					...
					<Link
						...
						className="absolute text-sm h-full bg-transparent w-full left-0 top-0 font-bold text-white z-0"></Link>
				</div>
			</div>
			<div>
				{direction !== "flex-col" ? (
					<h4 className="text-sm font-bold text-secondary max-w-48 overflow-hidden text-ellipsis mx-auto">
						{book.title}
					</h4>
				) : (
					<Link
						reloadDocument
						to={`/catalog/item/${book.id}`}
						className="text-sm font-bold text-secondary max-w-48 overflow-hidden text-ellipsis mx-auto">
						{book.title}
					</Link>
				)}

...
import { addCartItem, addNewCart, getCartById, updateCartItemQuantity } from "../../../features/cart/cart";
                ...
                <div className="flex flex-col gap-1">
                    ...
                    {carts.map((cart: CartType) => (
                        <h3 key={cart.id} onClick={() => handleAddCartItem(
                            ...
                            book.id,
                            1
                        )} className="p-1 text-white cursor-pointer text-start rounded-md bg-black overflow-hidden text-ellipsis">{cart.name}</h3>
                    ))}
                </div>
            )}
            {   displayNewCart &&
                <form className="relative flex flex-1 items-center [&>*]:outline-none">
                    ...
                    <button className="absolute bg-success-background right-0 my-auto text-success-text flex-1 rounded-full p-1"

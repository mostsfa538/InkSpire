...
function OrderDetails({ order }: { order: OrderType }) {
	...
	const handleCheckout = async () => {
		const response = await dispatch(
			checkoutOrder({ userId: order.user_id!, orderId: order.id! })
		);
	...
	return (
		<div className="flex flex-col gap-2 overflow-hidden">
			<div
				className={`relative flex gap-2 rounded-md transition-all ${orderBackground(
					order.order_status!
				)} border-2 hover:bg-gray-200 p-2`}>
				<div
					className="flex w-full h-full my-auto gap-2 cursor-pointer items-center justify-between text-nowrap transition-all rounded-md max-md:text-xs"
					onClick={() => setDisplayOrder(!displayOrder)}>
					<span className="h-full my-auto">
						Order Id: <span className="text-black">#{order.id}</span>
					</span>
					<span
						className={`absolute w-full h-3/5 overflow-hidden left-0 top-1/3 opacity-20 text-center text-5xl ${
							orderStatusText(order.order_status!).color
						} z-0 italic max-md:hidden`}>
					...
					{!(
						...
						<>
							{displayViewOrder.type === "add" ? (
								<button
									disabled={
										order.carts!.find((cart) => cart.id === cartToOrder?.id)
											? true
											: false
									}
									onClick={() =>
										handleAddCartToOrder(
											order.user_id!,
											cartToOrder!.id,
											order.id!
										)
								...
								displayViewOrder.type === "checkout" && (
									<button
										...
										className="bg-black text-white p-2 h-fit my-auto text-xs rounded-md font-semibold disabled:opacity-50">
										Checkout
									</button>
								)
							)}
			...
			<div
				className={`flex flex-col text-sm ${
					!displayOrder ? "h-0 py-0" : "h-96 py-2"
				} bg-gray-200 rounded-md px-2 max-h-fit overflow-hidden gap-1 transition-all ease-in-out duration-500 [&>*>span]:underline max-lg:text-xs`}>
				<span>
				...
				<div className="flex flex-col gap-2">
					...
					<div className="px-4 flex flex-col gap-2">
						{order.carts?.map((cart) => (
							<span
								key={cart.id}
								className="p-2 rounded-md bg-white flex items-center justify-between">

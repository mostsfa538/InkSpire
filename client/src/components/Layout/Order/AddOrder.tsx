...
function AddOrder({ orders }: { orders: OrderType[] }) {
	...
	const handleCreateOrder = (
		...
		e.preventDefault();
		if (address.length < 20) alert("Address is too short (min 20 characters)");
		if (phone.length < 5) alert("Phone is too short (min 5 characters)");
		dispatch(
			createNewOrder({
				userId,
				address,
	...
	return (
		<div className="flex flex-col gap-2">
			<div className={`p-2`}>
				{orders.length === 0 ? (
					<h1 className="text-xs text-center p-2 bg-gray-200">No Orders</h1>
				) : (
					orders.map((order) => <Order key={order.id} orders={[order]} />)
				)}
			</div>
			{displayForm && (
			...
			<button
				...
				className="text-white w-full p-2 text-sm flex items-center rounded-md justify-center gap-2 bg-black transition-all hover:bg-opacity-90">
				{displayForm ? (
					<>
						<CgClose />
						Close Form
